...
import com.hipaasafe.call_manager.helper.CometChatAudioHelper
import com.hipaasafe.call_manager.helper.OutgoingAudioHelper
...
class CometChatCallActivity : AppCompatActivity(), View.OnClickListener {
    ...
    private fun handleIntent() {
        val intent = intent
        if (intent.hasExtra(Constants.CometChatConstant.JOIN_ONGOING)) {
            isOngoing = intent.getBooleanExtra(Constants.CometChatConstant.JOIN_ONGOING, false)
        }
        if (intent.hasExtra(Constants.CometChatConstant.ID)) {
            val id = intent.getStringExtra(Constants.CometChatConstant.ID)
        }
        if (intent.hasExtra(Constants.CometChatConstant.SESSION_ID)) {
            sessionId = intent.getStringExtra(Constants.CometChatConstant.SESSION_ID)
        }
        if (intent.hasExtra(Constants.CometChatConstant.AVATAR)) {
            avatar = intent.getStringExtra(Constants.CometChatConstant.AVATAR)
        }
        if (intent.hasExtra(Constants.CometChatConstant.NAME)) {
            name = intent.getStringExtra(Constants.CometChatConstant.NAME)
        }
        if (!isOngoing) {
            try {
                isVideo = intent.action == CometChatConstants.CALL_TYPE_VIDEO
                isIncoming = intent.type == Constants.CometChatConstant.INCOMING
    ...
    private fun initView() {
        ...
        if (CometChatFeatureRestriction.isCallsSoundEnabled()) {
            cometChatAudioHelper!!.initAudio()
            val packageName = packageName
            notification =
                Uri.parse("android.resource://" + packageName + "/" + R.raw.incoming_call)
        }
    ...
    private fun setValues() {
        if (isOngoing) {
            ...
            if (CometChat.getActiveCall() != null) AppUtils.INSTANCE?.startCall(
                this,
...
//        callerAvatar!!.setAvatar(avatar)
    ...
    private fun rejectCall(sessionId: String?, callStatus: String) {
        CometChat.rejectCall(sessionId!!, callStatus, object : CallbackListener<Call?>() {
            ...
            override fun onError(e: CometChatException) {
                ...
                Toast.makeText(this@CometChatCallActivity, e.code, Toast.LENGTH_LONG).show()
            }
        })
    }
    ...
    private fun startCall(mainView: RelativeLayout?, call: Call) {
        ...
        AppUtils.INSTANCE?.startCall(this@CometChatCallActivity, call, mainView)
    }
