package com.hipaasafe.call_manager.listener
            ...
            override fun onIncomingCallReceived(call: Call) {
                if (CometChat.getActiveCall() == null) {
                    if (call.receiverType == CometChatConstants.RECEIVER_TYPE_USER) {
                        CometChatUtils.startCallIntent(
                            context!!, call.callInitiator as User, call.type,
                            false, call.sessionId
                        )
                    } else {
                        CometChatUtils.startGroupCallIntent(
                    ...
                    CometChat.rejectCall(
                        ...
                        object : CallbackListener<Call?>() {
                            override fun onSuccess(call: Call?) {
                                Toast.makeText(
                                    context,
                                    call?.sender?.name + " tried to call you",
                                    Toast.LENGTH_LONG
                                ).show()
                            }
...
//                            Toast.makeText(context, "Error:" + e.message, Toast.LENGTH_LONG).show()
                                if (context != null) {
                                    Toast.makeText(context, e.code, Toast.LENGTH_LONG).show()
                                }
                            }
                        })
                }
            ...
            override fun onOutgoingCallAccepted(call: Call) {
                if (CometChatCallActivity.mainView != null) {
                    ...
                    AppUtils.INSTANCE?.startCall(
                        CometChatCallActivity.callActivity!!,
                        call,
                        CometChatCallActivity.mainView
                    )
                }
            }
    ...
    fun makeCall(context: Context?, receiverId: String?, receiverType: String?, callType: String?) {
        CometChatUtils.initiatecall(context!!, receiverId, receiverType, callType)
    }
}